...
for (i in 1:no.of.folds) {
    fit = svdpc.fit(x[indx != i,], y[indx != i], center = TRUE, stripped = TRUE, ncomp = p)
    
    fit.coeff = array(fit$coefficients[,1,], dim = c(ncol(x), p))
...
for (k in 1:5) {
    ...
    fit = svdpc.fit(x[indx,], y[indx], ncomp = z, center = TRUE, stripped = TRUE)
    
    fit.coeff = fit$coefficients[, 1, z]
    
    y.val.hat = x[-indx,]%*%fit.coeff
    err.test = (y.val.hat - y[-indx])^2 %>% mean()
    
    est.test.err = est.test.err + err.test
